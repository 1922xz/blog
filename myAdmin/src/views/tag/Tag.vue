<template>
  <el-card class="main-card">
    ...
    />
    <!-- 批量删除对话框 -->
    <el-dialog :visible.sync="isDelete">
    ...
    <!-- 编辑对话框 -->
    <el-dialog :visible.sync="addOrEdit">
...
export default {
  ...
  methods: {
    ...
    searchTags() {
      this.current = 1;
      this.tagall1 = this.tagall.filter((e) => {
        return e.tagName.indexOf(this.keywords.trim()) != -1;
      });
      this.tagList = this.tagall1.slice(
    ...
    deleteTag(id) {
      ...
      this.axios.delete("/api/admin/deletetags", param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
    ...
    listTags() {
      this.axios.get("/api/admin/getTagManage").then(({ data }) => {
        this.tagall = data.data;
        this.count = data.data.length;
        this.tagList = this.tagall.slice(
          (this.current - 1) * this.size,
          this.current * this.size
        );
        this.loading = false;
    ...
    addOrEditTag() {
      ...
      this.axios.put("/api/admin/putTag", this.tagForm).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
            message: data.message,
