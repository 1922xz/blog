<template>
  <v-dialog v-model="forgetFlag" max-width="460">
    <v-card class="login-container" style="border-radius: 4px">
      <i class="float-right iconfont icon-close" @click="forgetFlag = false"/>
...
export default {
  ...
  methods: {
    ...
    sendCode() {
      this.countDown();
      sendCode({ username: this.username }).then((data) => {
        if (data.flag) {
    ...
    async forget() {
      ...
      await this.axios
        .put("http://localhost:3008/api/forget", user)
        .then((res) => {
          // console.log('忘记密码登录',res);
          if (res.flag) {
  ...
  computed: {
    forgetFlag: {
      set(value) {
        this.$store.state.user.forgetFlag = value;
      },
      get() {
        // console.log(this.$store.state);
        return this.$store.state.user.forgetFlag;
      },
    },
  },
  watch: {
